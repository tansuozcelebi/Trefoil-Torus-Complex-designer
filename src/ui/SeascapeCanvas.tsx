...
const SeascapeMaterial = shaderMaterial(
  {
    iTime: 0,
  ...
  glsl`
    ...
    // TDM Seascape adapted functions and constants
    ...
    float mapSea(vec3 p){
      ...
      float d; float h = 0.0;
      float SEA_TIME_LOCAL = SEA_TIME;
      for(int i=0;i<3;i++){
        d = sea_octave((uv+SEA_TIME_LOCAL)*freq, choppy);
        d += sea_octave((uv-SEA_TIME_LOCAL)*freq, choppy);
        h += d * amp;
...
extend({ SeascapeMaterial });

function SeascapePlane() {
  const ref = useRef<any>();

  useFrame(({ clock, size, mouse }) => {
    if (ref.current) {
      // three/drei shaderMaterial exposes props directly on the ref
  ...
  return (
    <mesh>
      ...
      <seascapeMaterial ref={ref} />
    </mesh>
  );
}

export default function SeascapeCanvas() {
  return (
